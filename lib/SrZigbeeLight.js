...
class SrZigbeeLight extends ZigBeeLightDevice {
  ...
  async _registerMeterPowerMeasurePower () {
    ...
    if (this.hasCapability('meter_power')) {
      ...
      await this.registerCapability('meter_power', CLUSTER.METERING, {
        get: 'currentSummationDelivered',
        report: 'currentSummationDelivered',
        reportParser: value => {
          this.log(`currentSummationDelivered report value`, value, meterFactory)
          return value * 0.01 / 36000.0
        },
        getParser: value => {
          this.log(`currentSummationDelivered getParser value`, value, meterFactory)
          return value * 0.01 / 36000.0
        },
        getOpts: {
          getOnStart: true,
          getOnOnline: true,
          pollInterval: 60000 * 30, // 30 minutes
        },
        reportOpts: {
          configureAttributeReporting: {
            minInterval: 1, // Minimally once every 300 seconds
    ...
    if (this.hasCapability('measure_power')) {
      ...
      await this.registerCapability('measure_power', CLUSTER.ELECTRICAL_MEASUREMENT, {
        ...
        report: 'activePower',
        reportParser: value => {
          this.log(`activePower report value`, value, measureFactory)
          return value * measureFactory
        },
        getParser: value => {
          this.log(`activePower getParser value`, value, measureFactory)
          return value * measureFactory
        },
        getOpts: {
          getOnStart: true,
          getOnOnline: true,
          pollInterval: 60000 * 5, // 1 minutes
        },
        reportOpts: {
          configureAttributeReporting: {
            minInterval: 1, // Minimally once every 300 seconds
